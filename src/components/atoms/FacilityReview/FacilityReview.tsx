...
const FacilityReview = ({ review }: FacilityReviewProps) => {
  return (
    <Container className="row border p-0 m-0">
      <Container className="col-12 col-lg-3 d-flex flex-column justify-content-start py-4 ps-2 pe-5 border-end">
        <p className="mb-1 fw-bold-600 text-nowrap">{review.name}</p>
        <p className="m-0">{review.createdAt ? review.createdAt : null}</p>
      </Container>
      <Container className="col-12 col-lg-2 d-flex flex-column justify-content-start align-items-center gap-3 py-4 px-5 border-end">
